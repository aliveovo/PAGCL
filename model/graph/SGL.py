...
from util.loss_torch import bpr_loss_WN, l2_reg_loss, InfoNCE
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                # align = 0.1 * alignment(user_emb, pos_item_emb)
                cl_loss = self.cl_rate * model.cal_cl_loss([user_idx,pos_idx],dropped_adj1,dropped_adj2)
                batch_loss = rec_loss + l2_reg_loss(self.reg, user_emb, pos_item_emb) + cl_loss
                # batch_loss =  rec_loss + l2_reg_loss(self.reg, user_emb, pos_item_emb) + cl_loss + align
...
class SGL_Encoder(nn.Module):
    def __init__(self, data, emb_size, drop_rate, n_layers, temp, aug_type):
        ...
        self.sparse_norm_adj = TorchGraphInterface.convert_sparse_mat_to_tensor(self.norm_adj).cuda(3)
    ...
    def random_graph_augment(self):
        ...
        return TorchGraphInterface.convert_sparse_mat_to_tensor(dropped_mat).cuda(3)
    ...
    def cal_cl_loss(self, idx, perturbed_mat1, perturbed_mat2):
        u_idx = torch.unique(torch.Tensor(idx[0]).type(torch.long)).cuda(3)
        i_idx = torch.unique(torch.Tensor(idx[1]).type(torch.long)).cuda(3)
