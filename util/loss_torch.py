...
def bpr_loss_2(user_emb, pos_item_emb, neg_item_emb):
    pos_score = torch.mul(user_emb, pos_item_emb).sum(dim=1)
...
def alignment(user_emb, pos_item_emb):
    x, y = F.normalize(user_emb, dim=-1), F.normalize(pos_item_emb, dim=-1)
...
def ce_loss(pos_out):
    pos_loss = F.binary_cross_entropy(pos_out.sigmoid(), torch.ones_like(pos_out), reduction='mean')
    # neg_loss = F.binary_cross_entropy(neg_out.sigmoid(), torch.zeros_like(neg_out), reduction='sum')
    return pos_loss
